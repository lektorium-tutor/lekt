...
class Renderer:
    ...
    def render_template(self, template_name: str) -> t.Union[str, bytes]:
        ...
        try:
            return self.__render(template)
        except (jinja2.exceptions.TemplateError, exceptions.LektError):
...
def check_is_up_to_date(root: str) -> None:
    if not is_up_to_date(root):
        fmt.echo_alert(
            ...
            f"v{current_version(root)} while the 'lekt' binary is at v{__version__}. You should upgrade "
...
def is_up_to_date(root: str) -> bool:
    """
    Check if the currently rendered version is equal to the current lekt version.
    """
