...
from lekt.plugins.base import PLUGINS_ROOT, PLUGINS_ROOT_ENV_VAR_NAME
...
@click.group(
    name="plugins",
    short_help="Manage Lekt plugins",
)
...
def enable(context: Context, plugin_names: list[str]) -> None:
    ...
    lekt_config.save_config_file(context.root, config)
    fmt.echo_info(
...
def disable(context: Context, plugin_names: list[str]) -> None:
    ...
    disabled: list[str] = []
    for plugin in lekt_config.get_enabled_plugins(config):
...
def install(names: list[str]) -> None:
    ...
    1. A plugin name from the plugin indexes (see `lekt plugins search`)
    2. A local file that will be copied to the plugins root
    3. An http(s) location that will be downloaded to the plugins root

    In cases 2. and 3., the plugin root corresponds to the path given by `lekt plugins
    printroot`.
    """
...
def find_and_install(names: list[str], pip_install_opts: t.List[str]) -> None:
    ...
    if pip_requirements:
        ...
        with tempfile.NamedTemporaryFile(
            prefix="lekt-reqs-", suffix=".txt", mode="w"
        ) as tmp_reqs:
...
def index_command() -> None:
    ...
    A plugin index is a list of Lekt plugins. An index can be public and shared with
    the community, or private, for instance to share plugins with a select group of
    users. Plugin indexes are a great way to share your plugins with other Lekt users.
    ...
    Plugin index cache is stored locally in the following subdirectory of the Lekt project environment:
