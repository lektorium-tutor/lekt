"""
List of all the action, filter and context names used across Lekt. This module is used
...
class Actions:
    ...
    #:
    #: - Python packages that declare a "lekt.plugin.v0" entrypoint.
    #: - Python packages that declare a "lekt.plugin.v1" entrypoint.
    #: - YAML and Python plugins stored in ~/.local/share/lekt-plugins (as indicated by ``lekt plugins printroot``)
    #: - When running the binary version of Lekt, official plugins that ship with the binary are automatically discovered.
    #:
    #: Discovering a plugin is typically done by the Lekt plugin mechanism. Thus, plugin
...
class Filters:
    ...
    #: List of folders to bind-mount in docker-compose containers, either in ``lekt local`` or ``lekt dev``.
    #:
    #: Many ``lekt local`` and ``lekt dev`` commands support ``--mounts`` options
    ...
    #: Modify existing settings, either from Lekt core or from other plugins. Beware not to override any
    ...
    #:    {{ "somevalue"|my_filter }}
    #:
    #: Note that Jinja2 filters are a completely different thing than the Lekt hook
    #: filters, although they share the same name.
    #:
    #: Out of the box, Lekt comes with the following filters:
    #:
...
class Contexts:
    """
    Here we list all the :py:class:`contexts <lekt.core.hooks.Context>` that are used across Lekt. It is not expected that
