...
def is_root() -> bool:
    """
    Check whether lekt is being run as root/sudo.
    """
...
def docker(*command: str) -> int:
    if shutil.which("docker") is None:
        raise exceptions.LektError(
            ...
            Please follow instructions from https://docs.docker.com/install/"
        )
...
def _docker_compose_command() -> Tuple[str, ...]:
    ...
    """
    if os.environ.get("LEKT_USE_COMPOSE_SUBCOMMAND") is not None:
    ...
    if shutil.which("docker") is not None:
        if (
            ...
            return ("docker", "compose")
    raise exceptions.LektError(
        "docker-compose is not installed. Please follow \
...
def execute_silent(*command: str) -> int:
    with subprocess.Popen(command) as p:
        try:
            ...
            p.kill()
            p.wait()
            raise exceptions.LektError(f"Command failed: {' '.join(command)}") from e
        if result > 0:
            raise exceptions.LektError(
...
def check_output(*command: str) -> bytes:
    ...
    try:
        ...
        raise exceptions.LektError(f"Command failed: {literal_command}") from e
...
def check_macos_docker_memory() -> None:
    ...
    allocated memory. Will raise LektError in case of parsing/loading error.
    """
    ...
    try:
        ...
        raise exceptions.LektError(f"Error accessing Docker settings file: {e}") from e
