...
from lekt import env, exceptions, fmt, hooks, serialize
...
class BasePlugin:
    """
    Lekt plugins are defined by a name and an object that implements one or more of the
    ...
    Lekt templates. For instance, to add "somecontent" to a template that includes '{{
    ...
    `command` (click.Command): if a plugin exposes a `command` attribute, users will be able to run it from the command line as `lekt pluginname`.
    """
...
class EntrypointPlugin(BasePlugin):
    ...
    def _version(self) -> t.Optional[str]:
        if not self.loader.dist:
            raise exceptions.LektError(f"Entrypoint plugin '{self.name}' has no dist.")
...
def _discover_v0_plugins() -> None:
    ...
    with hooks.Contexts.PLUGINS.enter():
        if "LEKT_IGNORE_ENTRYPOINT_PLUGINS" not in os.environ:
            with hooks.Contexts.PLUGINS_V0_ENTRYPOINT.enter():
                EntrypointPlugin.discover_all()
        if "LEKT_IGNORE_DICT_PLUGINS" not in os.environ:
