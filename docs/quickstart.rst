...
1. Install the latest stable release of Lekt from pip:

.. include:: download/pip.rst

Or `download <https://github.com/lektorium-tutor/tutor/releases>`_ the pre-compiled binary and place the ``tutor`` executable in your path:

.. include:: download/binary.rst

2. Run ``lekt local launch``
3. You're done!

**That's it?**

Yes :) This is what happens when you run ``lekt local launch``:
...
There's a lot more to Lekt than that! To learn more about what you can do with Lekt and Open edX, check out the :ref:`whatnext` section. If the launch installation method above somehow didn't work for you, check out the :ref:`troubleshooting` guide.
