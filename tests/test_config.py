...
from lekt import hooks, interactive
...
class ConfigTests(unittest.TestCase):
    def test_version(self) -> None:
        ...
        self.assertNotIn("LEKT_VERSION", defaults)

    def test_merge(self) -> None:
        config1: Config = {"x": "y"}
        config2: Config = {"x": "z"}
        lekt_config.merge(config1, config2)
        self.assertEqual({"x": "y"}, config1)

    def test_merge_not_render(self) -> None:
        config: Config = {}
        base = lekt_config.get_base()
        with patch.object(lekt_config.utils, "random_string", return_value="abcd"):
            lekt_config.merge(config, base)
    ...
    def test_update_twice_should_return_same_config(self, _: Mock) -> None:
        with temporary_root() as root:
            config1 = lekt_config.load_minimal(root)
            lekt_config.save_config_file(root, config1)
            config2 = lekt_config.load_minimal(root)
        ...
        with temporary_root() as rootdir:
            with patch.object(click, "prompt", new=mock_prompt):
                with patch.object(click, "confirm", new=mock_prompt):
                    config = lekt_config.load_minimal(rootdir)
    ...
    def test_is_service_activated(self) -> None:
        ...
        self.assertTrue(lekt_config.is_service_activated(config, "service1"))
        self.assertFalse(lekt_config.is_service_activated(config, "service2"))

    @patch.object(lekt_config.fmt, "echo")
    def test_json_config_is_overwritten_by_yaml(self, _: Mock) -> None:
        with temporary_root() as root:
            # Create config from scratch
            config_yml_path = os.path.join(root, lekt_config.CONFIG_FILENAME)
            config_json_path = os.path.join(
                root, lekt_config.CONFIG_FILENAME.replace("yml", "json")
            )
            ...
            # Reload and compare
            current = lekt_config.load_full(root)
